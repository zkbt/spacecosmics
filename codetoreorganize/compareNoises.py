...
def hogg(bin=1, number=25):
    ...
    cach = craftroom.oned.binto(mag[isort], scale*ach[isort], bin, robust=True)
    cexp = craftroom.oned.binto(mag[isort], scale*exp[isort], bin, robust=True)
    ...
    for i in range(len(alphas)):
        for j in range(len(Qs)):
            ...
            scale = 1e6
            bach = craftroom.oned.binto(mag[isort], scale*ach[isort], bin, robust=True)
            bexp = craftroom.oned.binto(mag[isort], scale*exp[isort], bin, robust=True)
...
def final():
    ...
    for j in range(len(keys)):
        ...
        bach = craftroom.oned.binto(mag[i], scale*ach[i], bin)
        bexp = craftroom.oned.binto(mag[i], scale*exp[i], bin)
        x= bexp[0]
        # plot the raw noises
